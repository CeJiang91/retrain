...
from postprocess import extract_picks, save_picks, save_picks_json, extract_amplitude, convert_true_picks, \
    calc_performance
            ...
            if data_reader_valid is not None:
                ...
                for _ in progressbar:
                    loss_batch, preds_batch, X_batch, Y_batch, fname_batch = sess.run(
                        [model.loss, model.preds, valid_batch[0], valid_batch[1], valid_batch[2]],
...
def test_fn(args, data_reader):
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        for _ in progressbar:
            ...
            progressbar.set_description(
                "{}, loss={:.6f}, mean loss={:6f}".format(args.mode, loss_batch, test_loss.value))
