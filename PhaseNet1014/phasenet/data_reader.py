    ...
    def random_shift(self, sample, itp, its, itp_old=None, its_old=None, shift_range=None):
        ...
        if (itp_old is None) and (its_old is None):
            ...
            if shift_range is None:
                ...
                if max(lo, shift_range[0])>min(hi + 1, shift_range[1]):
                    print(f"shift {lo},{shift_range[0]},{hi},{shift_range[1]}\n")
    ...
    def __getitem__(self, i):
        ...
        if np.random.random() < 0.95:
            try:#jc
                ...
                print(f"{base_name}, itp= {itp_list},its= {its_list}, unknown data problem, skipping..")
            ...
            ## noise
            #jc
            # print(f"{self.X_shape[0]},<=,{min(min(itp_list))}\n")
            # print(f"{itp_list}\n")
            # print(f"{len(sample)}\n")
            # breakpoint()
            # os._exit(0)
